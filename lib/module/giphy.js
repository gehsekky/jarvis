...
class Giphy extends JarvisModule {
  loadCommands(bot) {
    bot.commands.giphy = function (rtm, message, channel, user, args) {
      if (!args) {
        rtm.sendMessage('No search terms detected', channel.id)
        return
      }
      ...
      .then(result => {
        ...
        if (!result || !result.entity || !result.entity.data) {
          rtm.sendMessage('No results found', channel.id)
          return
        }
