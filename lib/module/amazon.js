...
class Amazon extends JarvisModule {
  ...
  loadCommands(bot) {
    bot.commands = Object.assign(bot.commands, {
      amazon(message, channel, user, args) {
        client.itemSearch({
          condition: 'New',
          searchIndex: 'All',
        ...
        .then((results) => {
          ...
          const random = _.random(results.length - 1)

          // build price field
          if (
            results[random].OfferSummary[0].LowestNewPrice
            && results[random].OfferSummary[0].LowestNewPrice[0].FormattedPrice[0]
          ) {
            fields.push({
              title: 'price',
